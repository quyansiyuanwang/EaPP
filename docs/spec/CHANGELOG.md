...
**协议版本**是 `eappVersion` 报告的那个号（v3.0 §19.3），当前为 **3.3.0**。
三个层文档的文件名是**冻结标识符**，不是协议版本 —— 它们标记该层文本首次冻结于哪个协议版本，
此后不再改名（v3.0 §18.1）。

| 层冻结标识符 | 文档 | 状态 | 该层的修订 |
|---|---|---|---|
| **v3.0.0-core** | `docs/spec/v3.0.0-core.md` | FROZEN | 补入 **C-7**（E-I）、§19.3 一致性声明接口的修正（E-J）、§18.1 版本规则的适用范围（E-L） |
| **v3.1.0-interaction** | `docs/spec/v3.1.0-interaction.md` | FROZEN | 恢复 §2.4（E-A）、补全 §2.2 转移表（E-B）、新增 §6.2 规则 7（E-H）、`E1-10` 更名为 `TR-9` |
| **v3.2.0-state** | `docs/spec/v3.2.0-state.md` | FROZEN | 补入 §5.4、§10.5、§15.1（E-G）、§12.4 能力矩阵改述（E-K） |
| — | `../analysis/DECISIONS-v3.2.0-r3.md` | 决议记录 | Final Review 的全部裁定（D-01…D-38、R-0…R-13） |

### 协议版本 3.3.0 相对 3.2.0

按 §18.1，**新增不变量**属于 `3.x.0`（次版本），因此：

```
3.2.0  →  3.3.0     新增 C-7（E-I）：Constraint 的匹配语义
```

E-A … E-K 中的其余各条属于 `3.0.x`（勘误、文本澄清，不改变语义），
它们就地修正在各层正文内，不单独产生协议版本。

`eappVersion` 报告的是**实现覆盖到的协议版本**：只实现 Composition Core 的实现
报告 `3.0.0`，覆盖三层的实现报告 `3.3.0`。这个区分是必要的 ——
两者都能满足 §19.3 的声明要求，而声明的强弱不同。
...
| E-M | v3.2 | §10.5 的 `CF-3` 要求非 CAS 策略"定义在 Extension 中"，而 `StateChannelConfig.conflictPolicy` 是冻结的字面量 `'cas'` —— 两种读法都能自称合规：把策略名塞进能力声明却仍以 `conflictPolicy: 'cas'` 配置 Channel（该字段因此成为假话），或认为 Core 需要放宽该字段。评审 [RFC 0001](../../rfcs/0001-extension-mechanism.md) 时两位独立实现者各自指出 | §10.5 写明 `CF-3` 的兑现方式：不要求 `StateChannel` 支持别的策略，非 CAS 策略由 Extension 提供自己的路径，`StateChannel` 保持只支持 CAS | 措辞澄清，不改变语义 |
| E-L | v3.0 | §18.1 的版本表只写「3.x.0 新增不变量」，没有说明它描述的是**协议版本**还是**层文档的版本**。三层各自冻结后，两者不再是同一个号 —— 于是往 Core 加一条不变量时，无法判断协议该不该升版。`eappVersion`（§19.3）报告的是协议版本，而文件名 `v3.0.0-core` 是冻结标识符，这一区分此前只存在于读者推断中 | §18.1 补入两者的关系与一组例子；`docs/spec/CHANGELOG.md` 的版本索引改为按层标识符组织，并单列协议版本一节 | 措辞澄清，不改变语义 |
