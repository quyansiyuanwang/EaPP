...
**本表描述的是协议版本**，即 §19.3 的 `eappVersion` 所报告的那个号。

三个层文档的文件名（`v3.0.0-core` / `v3.1.0-interaction` / `v3.2.0-state`）是**冻结标识符**，
不是协议版本：它们标记该层文本首次冻结于哪个协议版本，此后不再改名。因此

```
协议版本 3.0.0  ⟶  Composition Core 首次冻结，文档命名为 v3.0.0-core
协议版本 3.1.0  ⟶  Interaction 层加入
协议版本 3.2.0  ⟶  State Mode 加入
协议版本 3.3.0  ⟶  Core 新增一条不变量（C-7）；文档名不变
```

**向任一层新增不变量，都是协议版本的 `3.x.0` 变更**，与该层的冻结标识符无关。
只实现部分层的实现，其 `eappVersion` 报告的是它**覆盖到**的协议版本（§19.3）。
...
> 版本号写不进去，`I1`–`I7` 也写不进去 —— 而 v3.1 §15 定义了 `I1`–`I7`
