...
**取代**：`git show "9b78d40:tmp/draft/EaPP v3.2.0 State Mode — Freeze Candidate r2.md"`（草案原文）
...
r2 声称 6 P0 + 6 P1 已全部修正。逐行核对后，实际仍有 **40 条内部缺陷**（F-01…F-39，12 处 P0）
与 **12 处跨文档冲突**（X-1…X-12，5 处 P0）。本版本全部关闭。

### 0.1 改变语义内核的四条

| # | r2 | r3 |
|---|---|---|
| **C-1** | `Revision` 是 per-cell 版本号，`REV-7` 是"例外" | `Revision` 就是 Channel 内的**日志位置**，与 v3.1 的 `Cursor` 同域同型 ⇒ `REV-7` 成为 `CR-1` 的推论 |
| **C-2** | `delete` = `set({deleted:true})` | `delete` 是 Transport **一等原语** |
| **C-3** | `maxRevision` 由 cells 归约得出（恒真式，无法被违反） | `maxRevision` = 读取 cells **之前**的 head（可证伪） |
| **C-4** | CRDT 获得 `⚠️` 豁免，允许 eventual + revision | 取消豁免：`supportsStateRevision = false` 即禁止 CAS |

### 0.2 其余为消除歧义、补齐定义、修正自相矛盾

关键条目：`delete` 的错误码取决于 `expectedRevision` 形状（原来三处互斥）；
`get`/`list` 必须返回已逻辑删除的 cell（否则 `DEL-5` 的 no-op 分支不可达）；
`updatedBy` 的 actor 来源链（原来无任何填充路径，参考实现写死占位身份）；
`StateUpdateEvent` 必须实现完整 `v3.1 AckContext`（含 `nack()`）；
删除 `pending` 结构（与 v3.1 §6.4 的"显式 ack 可跳过"冲突）；
`WatchOptions` 从无定义到冻结；`durabilityBoundary` 从凭空引用到归属 v3.1；
Channel 创建路径补齐（原 r2 无法从 Binding 走到 StateChannel）。
...
本版本 MUST NOT 修改 `ChannelMode`（X-1）。
...
也无法充当 Cursor，r2 的 `readStateAfter` 正是因为这一点而不可实现。
...
**理由**：r2 把 `delete()` 实现成 `set({deleted:true})`，Transport 无法区分"删除"与"创建"，
导致 `EAPP_STATE_KEY_NOT_FOUND` **结构上不可产生**，而 DEL-4 与 §15 却要求它。

### 6.2 边界情况（完整表，取代 r2 的三行表）
...
**MUST NOT** 只提供 `ack()` —— 只提供 ack 的类型不是合法的 v3.1 `AckContext`（X-4 / E1-3）。

### 7.3 ack / nack 语义（关闭 F-14 / X-5）
...
**MUST NOT 引入 `pending` 结构。** r2 §14.2 的 `pending` + "只推进到第一个 PENDING 之前"
与 v3.1 §6.4「ack 一个更新的 cursor 意味着放弃中间未 ack 的消息」直接冲突（X-5）。
CR-3 禁止的是**隐式**跳过，显式 ack 更靠后的位置并放弃中间项是允许的。

### 7.4 初始位置（关闭 F-13 / F-19）
...
r2 §14.2 把 `_cursor` 留成 `undefined` 并指望"Transport 在首次迭代时提供 now 位置"，
这使 r2 自己的 SW-1 测试（`expect(w.cursor).toBeDefined()`）**必然失败**。
...
## 9. Snapshot / Restore（关闭 F-03 / F-04 / F-09 / F-29）
...
interface StateSnapshot {
  ...
  readonly pattern: StatePattern;   // 本快照的选择范围（r2 缺失）
...
**理由**：r2 的 `cells` 与 `maxRevision` 来自同一次读取，`cell.revision <= maxRevision`
是**恒真式**，Transport 可以返回任意不一致的 cell 集合而"满足" SNAP-1。
把 head 的读取放到 cell 读取**之前**，就打开了一个真实的竞态窗口，
使该不变量**可被违反、因而可被测试**。

r2 用 `'' as Revision` 作归约种子，零匹配时 `maxRevision === ''`——一个没有任何
Transport 能比较的非法值，违反了 SNAP-2。
...
**r2 的 `restore` 只写不删**，与 API-8「MUST overwrite current state」的两种读法并存；
本版本用显式 `mode` 消除歧义。
...
**理由**：r2 同时存在 `StateTransport.restoreState`（L801）与 StateChannel 的自建循环
（L1069-1081）两个竞争入口，关系未定义，可能双写。
...
> **`watch` 返回 Promise —— 相对 r2 的一处签名修订。**
> r2 §10.2 写的是同步的 `watch(...): StateWatcher`，但它同时要求初始 cursor 是**具体位置**
> （`SUB-9` / `SW-9`）。解析 `'latest'` 需要读取 Channel head，那是一次异步读取；
> 同步返回只能交出一个未解析的 cursor —— 而这正是 r2 自己的 SW-1 断言
> （`expect(w.cursor).toBeDefined()`）**永远无法通过**的原因。
> 二者不可兼得，本版本选择让 `watch` 异步，以换取一个真正可满足的 `SUB-9`。
...
> **TS-13** 关闭 r2 的 `` `${channel}:${key}` `` 缺陷：
> `(channel="a", key="b:c")` 与 `(channel="a:b", key="c")` 会命中同一个 cell，造成跨 channel 污染。

`readChangesAfter` 取代 r2 的 `readStateAfter`：r2 返回 `StateCell[]`（当前值），
**同一 key 的两次变更无法表达**，中间的变更永久丢失，cursor 语义因此不可实现。
返回**变更流**才能让"独立 cursor + per-update ack + 删除事件可观察"三者同时成立。
...
r2 给 CRDT 加了 `⚠️` 豁免，允许"eventual 的 revision + CAS"共存 —— 那等于允许一个
会**静默丢更新**的 CAS。CAS 的正确性就建立在全序之上，因此取消豁免，改为收紧能力标志。
...
> **CRDT 行的 `revision` 与 `snapshot` 从 r2 的 `⚠️` 改为 ❌**：
...
三份文档原本各自声明了一个同名 `interface EappError` 却从未定义类，
而参考实现处处 `new EappError(...)` —— X-10 据此收敛。
...
(IX-5 见 R-1：ChannelMode 从未被扩展，该条已删除。)
```
...
Tag:    v3.2.0-state
Status: FROZEN
Prev:   git show "9b78d40:tmp/draft/EaPP v3.2.0 State Mode — Freeze Candidate r2.md"
