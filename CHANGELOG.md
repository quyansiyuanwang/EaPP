...
## 3.6.0 — 装载，以及独立性实验带出的修正

标签 `spec-3.6.0`。**本版本的全部修正来自一次独立性实验，不是来自设计。**

两个实现者各自只读规范正文、彼此不可见，各写了一份实现与一套测试。结果：

```
原始交叉运行      甲的实现上跑乙的测试、乙的实现上跑甲的测试 → 两侧都在第一行失败
                  （甲的装载入口叫 registerPlugin，乙的叫 register；规范从未定义它）
手工桥接装载面后   甲的 Core 测试在乙的实现上 → 21 通过 / 52 失败
```

正文没有把语义钉死到可替换的程度，而失败集中在正文没有写下的形状上。

### 新增

- **§8.4 装载。** `register(descriptor) -> PluginRef`、`unregister(plugin) -> ()`，
  以及 `PluginRef` 的字段表。此前"Plugin 如何进入运行时"只由 §15 的 Bootstrap Runtime
  覆盖"第一个 Plugin"，其余部分没有定义 —— 两位实现者都把它当作缺口，各自发明了一套，
  因此各自的测试无法跑在对方上。
- 不变量 `P-5`…`P-8`、`OP-10`；`CS1` 增加装载操作。

### 变更

| 位置 | 原文的问题 |
|---|---|
| `OP-4` | 写着"两个操作的 `to` MUST 指向同一方"，与 §9.1（`to` 是消费方）及 §52 自己的等价式矛盾。两个 `from` 指向相反的一方 |
| `OP-7` | 原先要求"两份实现能只经由表面互通"。那是**单份实现无法提供判定**的条款，而 §3.1 要求每条不变量都有可执行的判定 |
| `OP-9` | 枚举漏了 `I2`，而 `CS4` 蕴含自含 `I2`，于是"声明 `I2` 却不声明 `CS4`"能通过 |
| §53 `CS5` | 蕴含自一列写了 `RQ`，而 `RQ-*` 是不变量组，不是 §3.2 定义的等级；改为 `I1` |
| 附录 D | `EAPP_TIMEOUT` 的分卷写成"插件开发表面"，与"条件只写在归属分卷"冲突；改为 Interaction Layer |
| §18 | `EAPP_IDENTITY_INVALID` 未覆盖 `instance` 为空与 `ID-6`；`EAPP_CAPABILITY_NOT_FOUND` 未覆盖 `C-1`/`C-2` |
| §18 | `EAPP_PLUGIN_INACTIVE` 标注为**本版本不可达** —— 没有任何操作以"该 Plugin 处于 ACTIVE"为前提 |

### 一致性

```
不变量            226 / 226    全部在正文中被陈述
错误码            36            0 个没有产生条件
章节              54            编号连续；113 处 § 引用全部解析
```

### 未收口

`rfcs/0002` 与 `rfcs/0003` 各收到**一份**独立实现者的评审（§2.3 要求两份）。
两份评审都是 `Accept with changes`。本版本回应了其中可直接落笔的部分；
评审指出的其余缺口列在两份提案的「未解决的问题」里，未在本版本处理。
...
> **本版本与 `3.4.0` 都尚未取得规范 §2.3 要求的评审。**

### 新增

- **每个错误码的产生条件。** §18 / §33 / §47 各增加一张表，逐条给出该码在何种条件下 MUST 被返回。
  在此之前，全部 36 个码只在错误模型里被登记名字，其中 **17 个在正文的任何地方都没有触发条款** ——
  两个实现可以为同一个码规定不同的条件，而两者都能自称合规。合规的判据是不变量，
  而不变量没有提到这些码。
- 不变量 `ER-1`（实现必须使用规范为该条件规定的码）与 `ER-2`（每个登记码必须有产生条件）。
- `check:spec` 的第八条规则，机械检查 `ER-2`：附录 D 登记的每个码 MUST 也在正文中出现。

### 变更

- 附录 D 不再分「有触发条款」与「仅登记」两类 —— 两类现在都只指回 §18 / §33 / §47 的表。
  它仍然不重复那些条件：条件只有一处，附录只回答「一共定义了哪些码」。

### 一致性

```
不变量            221 / 221    全部在正文中被陈述
错误码            36 个        0 个没有产生条件
```

### 未收口

`ER-1` 无法由 `check:spec` 判定 —— 它约束的是实现的行为，需要实现方的测试。
本次收口解决的是「条件是否被写下来」，不是「实现是否照做」。
...
附录 D 当时把这两类分开列出。**该缺口已在 `3.5.0` 收口。**
